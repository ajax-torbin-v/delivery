...
class OrderRepositoryTest : AbstractMongoTestContainer {
    ...
    fun `deleteById should delete order by id`() {
        ...
        // AND THEN
        orderRepository.existsById(savedOrder?.id.toString())
            .test()
            .expectNext(false)
            .verifyComplete()
    }
    ...
    fun `updateOrderStatus should update the order status`() {
        ...
        actual
            .test()
            .assertNext { order -> assertEquals(MongoOrder.Status.COMPLETED, order.status) }
            .verifyComplete()
    }
    ...
    fun `findAllByUserId should return all orders`() {
        ...
        actual
            .test()
            .assertNext { order -> assertEquals(savedOrder, order) }
            .verifyComplete()
    }
}
